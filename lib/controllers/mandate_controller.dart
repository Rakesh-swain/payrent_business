...
class MandateController extends GetxController {
  ...
  // API endpoints
  static const String _baseUrl = 'http://172.31.248.205:7777';
  ...
  DateTime calculateEndDate(DateTime startDate, int noOfInstallments, String frequency) {
  ...
  Future<MandateModel?> createMandate({
    ...
    try {
      ...
      final endDate = calculateEndDate(startDate, noOfInstallments, paymentFrequency);
  ...
  Future<Map<String, dynamic>?> _callMandateDetailsApi(MandateModel mandate) async {
    try {
  ...
  List<MandateModel> getActiveMandates() {
    return _mandates.where((mandate) => mandate.mmsStatus == 'ACCEPTED').toList();
  }
  
  /// Get pending mandates
  List<MandateModel> getPendingMandates() {
    return _mandates.where((mandate) => mandate.mmsStatus == 'PENDING').toList();
  }
